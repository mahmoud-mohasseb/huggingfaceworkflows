...
export const NodeInspector: React.FC<NodeInspectorProps> = ({
  ...
  return (
    <aside className="w-96 border-l border-slate-800/80 bg-slate-950/95 backdrop-blur-2xl h-full flex flex-col overflow-hidden z-20 shadow-2xl select-none">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-4 pb-12 overscroll-contain custom-sidebar-scroll">
        {/* Tab 1: Parameters Schema Form */}
        {activeTab === 'params' && (
          <div className="space-y-4">
            {/* Primary High-Visibility Apply Model Changes Card */}
            <div className="bg-slate-900/90 border-2 border-violet-500/40 hover:border-violet-500/70 rounded-2xl p-4 space-y-3 shadow-lg shadow-violet-950/40 transition-all">
              ...
              <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800 text-[11px] font-mono text-slate-300 flex items-center justify-between">
                <span className="text-slate-400 text-[10px]">Active Model:</span>
                <span className="text-amber-300 font-bold truncate max-w-[200px]" title={currentModelId}>
                  {currentModelId}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-0.5">
                <button
                  ...
                  className={`py-2.5 px-3 rounded-xl text-xs font-bold shadow-md transition-all flex items-center justify-center gap-2 ${
                    isSaved
                ...
                >
                  ...
                  <span>{isSaved ? 'Applied! ✅' : 'Apply Model'}</span>
                </button>
                ...
                {onRunSingleNode && (
                  <button
                    ...
                    className="py-2.5 px-3 bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                  >
          ...
          <div className="space-y-3">
            ...
            {data.lastOutput ? (
              ...
              <div className="text-center py-12 space-y-2">
                ...
                <div className="text-[10px] text-slate-500">Click &ldquo;Test Model&rdquo; above to run this step!</div>
              </div>
            )}
          </div>
        )}
